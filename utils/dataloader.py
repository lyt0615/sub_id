...
def make_trainloader(ds, batch_size=16, num_workers=1, train_size=0.8, seed=42, tune=False, pool_dim=None):
    ...
    if train_size is None:
        train_x = np.load(f'datasets/{ds}/train_x.npy')
        train_y = np.load(f'datasets/{ds}/train_y.npy')
        val_x = np.load(f'datasets/{ds}/eval_x.npy')
        val_y = np.load(f'datasets/{ds}/eval_y.npy')
